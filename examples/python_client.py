    ...
    async with client:
        ...
        if job_id:
            ...
            try:
                ...
                # Handle it as a list directly
                if isinstance(resources, (list, tuple)):
                    contents = list(resources)
                elif hasattr(resources, "contents"):
                    # Fallback: if it's a ReadResourceResult object with .contents
                    contents = resources.contents  # type: ignore
                elif isinstance(resources, dict) and "contents" in resources:
                    contents = resources.get("contents")  # type: ignore
                else:
                ...
                if not contents:
                    ...
                    content = contents[0]
                    # handle dataclass/object typed TextContent
                    # TextContent is already imported at the top of the file
                    if isinstance(content, TextContent):
